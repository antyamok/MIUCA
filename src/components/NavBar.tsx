...
const Navbar: React.FC = () => {
  ...
  const textColorClass = () =>
    isDashboard || isScrolled ? 'text-[#2A11D8]' : 'text-[#F7F4F2]';

  const logoSizeClass = () => {
    const isSpecial = isHomePage || isPricingPage || isAboutPage || isContactPage;
    return isSpecial && !isScrolled ? 'h-20 w-auto' : 'h-10 w-auto';
  };
